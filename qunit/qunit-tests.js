...
function loadSPtests() {
  ...
  $q(function() {
    ...
    $q.when( getSharePointMajorVersion() ).done(function(spversion) {
      ...
      /*document.querySelector('#testframe').style.width="100vw";
      document.querySelector('#testframe').style.height="50vh";*/
      ...
      $(frameDoc).ready(function() {
        ...
        test('modals', function(assert) {
          ...
          var doneShowModalDialogTitle = assert.async();
          var doneShowModalDialogBody = assert.async();
          var doneGetModalDialog = assert.async();
          var doneCloseModalDialog = assert.async();
          var doneNotify = assert.async();
          var doneRemoveNotify = assert.async();
          $SP().showModalDialog({
            id:"QUnitModal",
            title:"ModalTitle",
            html:"<p id='modalbody'>ModalBody</p>",
            onload:function() {
              assert.ok(frameDoc.querySelector('#dialogTitleSpan').innerHTML === "ModalTitle", ".showModalDialog() title")
              doneShowModalDialogTitle();
              assert.ok(frameDoc.querySelector('#modalbody').innerHTML === "ModalBody", ".showModalDialog() body")
              doneShowModalDialogBody();
              setTimeout(function() {
                var modal = $SP().getModalDialog('QUnitModal');
                if (modal) {
                  assert.ok(true, ".getModalDialog()")
                  doneGetModalDialog()
                  $SP().closeModalDialog(modal, true);
                } else {
                  assert.ok(false, ".getModalDialog()")
                  doneGetModalDialog()
                  assert.ok(false, ".closeModalDialog()")
                  doneCloseModalDialog();
                }
              }, 1200)
            },
            callback:function(res) {
              assert.ok(res === true, ".closeModalDialog()")
              doneCloseModalDialog();
            }
          });
          ...
          setTimeout(function() {
            assert.ok($('#notificationArea').find('span:last').text() === "Hello World", ".notify()");
            doneNotify();
            $SP().removeNotify("QUnitNotify", {all:true});
            setTimeout(function() {
              assert.ok($('#notificationArea').find('span:last').text() === "", ".removeNotify()");
              doneRemoveNotify();
            }, 1500)
          }, 1500)
        })
