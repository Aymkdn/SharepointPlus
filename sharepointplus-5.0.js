 ...
 */
var SPArrayChunk = function (b, e) {
  var d = []
  for (var c = 0, a = b.length; c < a; c += e) {
    d.push(b.slice(c, c + e))
  }
  return d
}
 ...
 */
var SPExtend = function () {
  var r, t, n, o, e = arguments[0] || {},
    f = 1,
    i = arguments.length,
    u = !1,
    y = function (r) {
      if (r === null || typeof r !== 'object' || r.nodeType || r !== null && r === r.window) return !1
      try {
        if (r.constructor && !this.hasOwnProperty.call(r.constructor.prototype, 'isPrototypeOf')) return !1
      } catch (t) {
        return !1
      }
      return !0
    }
  for (typeof e === 'boolean' && (u = e, e = arguments[f] || {}, f++), typeof e !== 'object' && typeof e !== 'function' && (e = {}), !1; i > f; f += 1) {
    if ((r = arguments[f]) !== null) { for (t in r) e !== r[t] && typeof e[t] === 'undefined' && (u && r[t] && (y(r[t]) || (n = Array.isArray(r[t]))) ? (n ? (n = !1, o = e[t] && Array.isArray(e[t]) ? e[t] : []) : o = e[t] && y(e[t]) ? e[t] : {}, e[t] = SPExtend(u, o, r[t])) : void 0 !== r[t] && (e[t] = r[t])) }
  }
  return e
}
...
(function (window, document, undefined) {
  ...
  SharepointPlus.prototype = {
    ...
    */
    hasREST: function (settings) {
      ...
      return _this._promise(function (prom_resolve) {
        ...
        var hasREST, needAjax = (!!(settings.url || !_SP_ISBROWSER || typeof SP === 'undefined'))
        if (!needAjax) {
    ...
    */
    ajax: function (settings) {
      ...
      ! function (e, t) {
        function n (e) {
          return e && t.XDomainRequest && !/MSIE 1/.test(navigator.userAgent) ? new XDomainRequest() : t.XMLHttpRequest ? new XMLHttpRequest() : void 0
        }

        function o (e, t, n) {
          e[t] = e[t] || n
        }
        var r = ['responseType', 'withCredentials', 'timeout']
        e.ajax = function (e, a) {
          function s (e, t) {
            return function () {
              p || (a(void 0 === c.status ? e : c.status, c.status === 0 ? 'Error' : c.response || c.responseText || t, c), p = !0)
            }
          }
          var u = e.headers || {},
            i = e.body,
            d = e.method || (i ? 'POST' : 'GET'),
            p = !1,
            c = n(e.cors)
          c.open(d, e.url, !0)
          var l = c.onload = s(200)
          c.onreadystatechange = function () {
            c.readyState === 4 && l()
          }, c.onerror = s(null, 'Error'), c.ontimeout = s(null, 'Timeout'), c.onabort = s(null, 'Abort'), i && (t.FormData && i instanceof t.FormData || o(u, 'Content-Type', 'application/x-www-form-urlencoded'))
          for (var f, v = 0, g = r.length; g > v; v++) f = r[v], void 0 !== e[f] && (c[f] = e[f])
          for (var f in u) u[f] !== !1 && c.setRequestHeader(f, u[f])
          return e.onprogress && c.upload.addEventListener('progress', e.onprogress, !1), e.getXHR && e.getXHR(c), c.send(i), c
        }, t.nanoajax = e
      }({}, (function () {
        return this
      }()))
    ...
    */
    ...
    webService: function (options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        options.soapAction = (options.soapAction !== false)
        ...
        _this.ajax(params).then(function (data) {
          prom_resolve(data)
        }, function (rej) {
          prom_reject(rej)
        })
    ...
    */
    _parseOn: function (q) {
      var factory = [],
        i = 0,
        mtch, queryString = q.replace(/(\s+)?(=)(\s+)?/g, '$2').replace(/==/g, '=').split(' AND ')
    ...
    */
    get: function (options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        setup.useIndexForOrderBy = (setup.useIndexForOrderBy === true)
        setup.groupby = setup.groupby || ''
        setup.rowlimit = setup.rowlimit || 0
        setup.whereEscapeChar = (setup.whereEscapeChar !== false)
        setup.paging = (setup.paging === true)
        ...
        setup.calendar = (setup.calendar === true)
        ...
        if (setup.view !== '') {
          ...
          _this.view(setup.view).then(function (view) {
            ...
            delete setup.view
            _this.get(setup).then(function (res) {
              prom_resolve(res)
            }, function (rej) {
              prom_reject(rej)
            })
        ...
        var fields = '',
          i, orderby = '',
          fieldsDir, direction, splt, groupby = '',
          gFields, tmpFields, body = '',
          viewAttr, where = '',
          whereDateRanges
        ...
        if (setup.queryOptions === undefined) {
          ...
          if (setup.folderOptions) {
            switch (setup.folderOptions.show) {
              case 'FilesAndFolders_Recursive':
                viewAttr = 'RecursiveAll'
                break
              case 'FilesOnly_InFolder':
                viewAttr = 'FilesOnly'
                break
              case 'FilesAndFolders_InFolder':
                viewAttr = ''
                break
          ...
          } else { setup._queryOptions += '<ViewAttributes Scope="Recursive"></ViewAttributes>' }
        ...
        _this.ajax({
          ...
          aReturn = fastMap(rows, function (row) {
            return myElem(row)
          })

            // if setup.results length is bigger than 0 then it means we need to add the current data
          if (setup.results.length > 0) { for (i = 0, stop = aReturn.length; i < stop; i++) setup.results.push(aReturn[i]) }

            // depending of the setup.nextWhere length we update the progress
          if (typeof setup.originalWhere !== 'string') { setup.progress(setup.originalWhere.length - setup.nextWhere.length, setup.originalWhere.length) }
          ...
          if (setup.join) {
                ...
                if (wh.length <= _SP_MAXWHERE_ONLOOKUP) {
                  ...
                  if (setup.join.where) {
                    if (Array.isArray(setup.join.where)) {
                      setup.join.where.forEach(function (e, i) {
            ...
            sp.get(setup.join).then(function (res) {
              prom_resolve(res)
            }, function (rej) {
              prom_reject(rej)
            })
    ...
    getContentTypeInfo: function (contentType, options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
          ...
          _this.getContentTypes(options).then(function (types) {
            ...
            for (var i = types.length; i--;) {
              if (types[i]['Name'] === contentType) {
                _this.getContentTypeInfo(types[i]['ID'], options).then(function (res) {
                  prom_resolve(res)
                }, function (rej) {
                  prom_reject(rej)
                })
        ...
        _this.ajax({
          ...
          var aReturn = [],
            i, j, a, r, k, q, arr = data.getElementsByTagName('Field'),
            index = 0,
            aIndex, attributes, attrName, lenDefault, attrValue, nodeDefault
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        _this.ajax({
          ...
          var aReturn = [],
            ...
            index = 0,
            aIndex, attributes, attrName, attrValue, lenDefault, nodeDefault, i, j, a, r, k, nName, nValue
    ...
    view: function (viewID, options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
          ...
          _this.views().then(function (views) {
            ...
            for (i = views.length; i--;) {
              if (views[i].Name === viewID) {
                _this.view(views[i].ID).then(function (res) {
                  prom_resolve(res)
                }, function (rej) {
                  prom_reject(rej)
                })
        ...
        _this.ajax({
          ...
          var node = data.getElementsByTagName('View'),
            i, where
    ...
    views: function (options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        _this.ajax({
          ...
          var aReturn = [],
            arr = data.getElementsByTagName('View'),
            i = 0,
            found = false
          for (; i < arr.length; i++) {
            aReturn[i] = {
    ...
    lists: function (setup) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        _this.ajax({
          ...
          var aReturn = [],
            arr = data.getElementsByTagName('List'),
            i, j, attributes
          for (i = 0; i < arr.length; i++) {
            aReturn[i] = {}
            attributes = arr[i].attributes
    ...
    add: function (items, options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        var itemsLength = items.length,
          nextPacket, cutted, itemKey, itemValue, it, i
        ...
        setup.progressVar = setup.progressVar || {
          ...
          passed: [],
          failed: [],
          eventID: 'spAdd' + ('' + Math.random()).slice(2)
        }
        ...
        _this.ajax({
          ...
          var result = data.getElementsByTagName('Result'),
            len = result.length,
            passed = setup.progressVar.passed,
            failed = setup.progressVar.failed,
            i, rows
          for (i = 0; i < len; i++) {
          ...
          if (setup.progressVar.current < setup.progressVar.max) {
            if (_SP_ADD_PROGRESSVAR[setup.progressVar.eventID]) {
              _SP_ADD_PROGRESSVAR[setup.progressVar.eventID](setup).then(function (res) {
                prom_resolve(res)
              }, function (rej) {
                prom_reject(rej)
              })
    ...
    update: function (items, options) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        setup.progressVar = setup.progressVar || {
          ...
          passed: [],
          failed: [],
          eventID: 'spUpdate' + ('' + Math.random()).slice(2)
        }
        // we cannot add more than 15 items in the same time, so split by 15 elements
        // and also to avoid surcharging the server
        if (itemsLength > setup.packetsize) {
          ...
          .then(function (data) {
            ...
            if (setup.progressVar.current < setup.progressVar.max) {
              if (_SP_UPDATE_PROGRESSVAR[setup.progressVar.eventID]) {
                _SP_UPDATE_PROGRESSVAR[setup.progressVar.eventID](setup).then(function (res) {
                  prom_resolve(res)
                }, function (rej) {
                  prom_reject(rej)
                })
    ...
    moderate: function (items, setup) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        setup.progressVar = setup.progressVar || {
          ...
          passed: [],
          failed: [],
          eventID: 'spModerate' + ('' + Math.random()).slice(2)
        }
        // we cannot add more than 15 items in the same time, so split by 15 elements
        // and also to avoid surcharging the server
        if (itemsLength > 15) {
        ...
        _this.ajax({
          ...
          if (setup.progressVar.current < setup.progressVar.max) {
            if (_SP_MODERATE_PROGRESSVAR[setup.progressVar.eventID]) {
              _SP_MODERATE_PROGRESSVAR[setup.progressVar.eventID](setup).then(function (res) {
                prom_resolve(res)
              }, function (rej) {
                prom_reject(rej)
              })
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (setup.where) {
          ...
          _this.get({
            ...
            _this.remove(aItems, setup).then(function (res) {
              prom_resolve(res)
            }, function (rej) {
              prom_reject(rej)
            })
        ...
        setup.progressVar = setup.progressVar || {
          ...
          passed: [],
          failed: [],
          eventID: 'spRemove' + ('' + Math.random()).slice(2)
        }
          ...
          if (setup.progressVar.current < setup.progressVar.max) {
            if (_SP_REMOVE_PROGRESSVAR[setup.progressVar.eventID]) {
              if (setup.useCallback) _SP_REMOVE_PROGRESSVAR[setup.progressVar.eventID](setup)
                ...
                _SP_REMOVE_PROGRESSVAR[setup.progressVar.eventID](setup).then(function (res) {
                  prom_resolve(res)
                }, function (rej) {
                  prom_reject(rej)
                })
    ...
    usergroups: function (username, setup) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!setup.url) {
          _this.getURL()
            .then(function (url) {
              return _this.usergroups(username, {
                url: url
              })
            })
            .then(function (res) {
        ...
        _this.get({
          ...
          .then(function (d) {
            ...
            _this.ajax({
              ...
              // we want to use myElem to change the getAttribute function
              var res = {},
                i, row, rows = data.getElementsByTagName('WorkflowTemplate')
              if (rows.length === 0) {
                ...
                for (i = 0; i < rows.length; i++) {
                  ...
                  res.instances.push({
                    ...
                    'Status1': {
                      ...
                      'text': _this.workflowStatusToText(row.getAttribute('Status1'))
                    },
                    'Status2': {
                      'code': row.getAttribute('Status2'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status2'))
                    },
                    'Status3': {
                      'code': row.getAttribute('Status3'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status3'))
                    },
                    'Status4': {
                      'code': row.getAttribute('Status4'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status4'))
                    },
                    'Status5': {
                      'code': row.getAttribute('Status5'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status5'))
                    },
                    'Status6': {
                      'code': row.getAttribute('Status6'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status6'))
                    },
                    'Status7': {
                      'code': row.getAttribute('Status7'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status7'))
                    },
                    'Status8': {
                      'code': row.getAttribute('Status8'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status8'))
                    },
                    'Status9': {
                      'code': row.getAttribute('Status9'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status9'))
                    },
                    'Status10': {
                      'code': row.getAttribute('Status10'),
                      'text': _this.workflowStatusToText(row.getAttribute('Status10'))
                    },
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!setup.fileRef && !setup.workflowID) {
          _this.getWorkflowID({
            ...
            _this.startWorkflow(setup).then(function (res) {
              prom_resolve(res)
            }, function (rej) {
              prom_reject(rej)
            })
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        SP.SOD.executeFunc("sp.js", "SP.ClientContext", function () { // eslint-disable-line
          ...
          SP.SOD.executeFunc('sp.workflowservices.js', "SP.WorkflowServices.WorkflowServicesManager", function () { // eslint-disable-line
            ...
            context.executeQueryAsync(function () {
              var subsEnum = subscriptions.getEnumerator(),
                sub
              ...
              if (setup.parameters) {
                ...
                for (i = 0; i < setup.parameters.length; i++) { initiationParams[setup.parameters[i].name] = setup.parameters[i].value }
              }
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!setup.url) {
          _this.getURL()
            .then(function (url) {
              setup.url = url
              return _this.distributionLists(username, setup)
            })
            .then(function (res) {
              prom_resolve(res)
            })
        ...
        _this.ajax({
          ...
          if (!found) {
            _SP_CACHE_DISTRIBUTIONLISTS.push({
              user: username,
              url: setup.url,
              data: aResult
            })
        ...
        if (!setup.url) {
          if (!_this.url) {
            _this.getURL()
              .then(function (url) {
                setup.url = url
                return _this.groupMembers(groupname, setup)
              })
              .then(function (res) {
                prom_resolve(res)
              })
    ...
    isMember: function (setup) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        _this.usergroups(setup.user, {
          ...
          return _this.groupMembers(setup.group, {
            cache: setup.cache
          })
        }).then(function (m) {
          ...
          return _this.distributionLists(setup.user, {
            cache: setup.cache
          })
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!setup.url) {
          _this.getURL()
            .then(function (url) {
              setup.url = url
              return _this.people(username, setup)
            })
            .then(function (res) {
              prom_resolve(res)
            })
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!setup.url) {
          _this.getURL()
            .then(function (url) {
              setup.url = url
              return _this.getUserInfo(username, setup)
            })
            .then(function (res) {
              prom_resolve(res)
            })
    ...
    regionalSettings: function (url) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!url) {
          _this.getURL()
            .then(function (url) {
              return _this.regionalSettings(url)
            })
            .then(function (res) {
              prom_resolve(res)
            })
    ...
    regionalDateFormat: function (url) {
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!url) {
          _this.getURL()
            .then(function (url) {
              return _this.regionalDateFormat(url)
            })
            .then(function (res) {
              prom_resolve(res)
            })
      ...
      return _this._promise(function (prom_resolve, prom_reject) {
        ...
        if (!setup.url) {
          _this.getURL()
            .then(function (url) {
              setup.url = url
              return _this.addressbook(username, setup)
            })
            .then(function (res) {
              prom_resolve(res)
            })
    ...
    */
    getLookup: function (str) {
      ...
      if (a.length <= 2) {
        ...
        return {
          id: str.replace(/([0-9]+;#)([^;]+)/g, '$1').replace(/;#;#/g, ',').slice(0, -2).split(','),
    ...
    */
    toXSLString: function (str) {
      ...
      var FullEscape = function (strg) {
        ...
        var rstr = '',
          i, c, num, temp, hexString, tmpStr, k
    ...
    */
    notify: function (message, options) {
      ...
      options.override = (options.override === true)
      options.overrideAll = (options.overrideAll === true)
      options.overrideSticky = (options.overrideSticky !== false)
      options.sticky = (options.sticky === true)
      ...
      if (_SP_NOTIFY_READY === false) {
        ...
        if (_SP_NOTIFY.length > 0) {
          ...
          } else if (options.override) { _this.removeNotify(_SP_NOTIFY[_SP_NOTIFY.length - 1].name) }
        }
      ...
      if (!options.sticky) {
        setTimeout(function () {
          _this.removeNotify(options.name, {
            timeout: true
          })
    ...
    */
    removeNotify: function (name, options) {
      ...
      options.timeout = (options.timeout === true)

      // make sure we are ready
      if (_SP_NOTIFY_READY === false && _SP_NOTIFY_QUEUE.length > 0) {
        setTimeout(function () {
          _this.removeNotify(name, options)
        }, 150)
        return _this
      }
      ...
      if (options.all === true) {
        ...
        while (_SP_NOTIFY.length > 0) {
          ...
          if (options.includeSticky === false && notif.options.sticky === true) a.push(notif)
            ...
            setTimeout(function () {
              notif.options.after.call(_this, notif.name, false)
            }, 150)
          }
        }
        ...
        for (var i = 0, len = _SP_NOTIFY.length; i < len; i++) {
          if (_SP_NOTIFY[i].name == name) {
            ...
            SP.UI.Notify.removeNotification(notif.id); // eslint-disable-line
            setTimeout(function () {
              notif.options.after.call(_this, notif.name, options.timeout)
            }, 150)
            return _this
          }
        }
    ...
    showModalDialog: function (options) {
      ...
      function iFrameReady (a, b) {
        ...
        function f () {
          this.readyState === 'complete' && e.call(this)
        }
        ...
        function h () {
          ...
          b.URL.indexOf('about:') !== 0 ? b.readyState === 'complete' ? e.call(b) : (g(b, 'DOMContentLoaded', e), g(b, 'readystatechange', f)) : c = setTimeout(h, 1)
        }
        var c, d = !1
        g(a, 'load', function () {
      ...
      options.closePrevious = (options.closePrevious === true)
   ...
   **/
  var myElem = (function () {
    var myElem = function (elem) {
        return new MyElemConstruct(elem)
      },
      MyElemConstruct = function (elem) {
        this.mynode = elem
  ...
  var extendMyObject = function (arr) {
    this.attributes = arr
  }
  extendMyObject.prototype.getAttribute = function (attr) {
    return this.attributes[attr]
  }
  extendMyObject.prototype.getAttributes = function () {
