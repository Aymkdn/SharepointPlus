...
export default function parse(q, escapeChar) {
    ...
    switch (letter) {
      case "(":
        ...
        while (parenthesis.open > 0 && i < limitMax) {
          ...
          else if (!ignoreNextChar && (charAtI == "'" || charAtI == '"')) openedApos = !openedApos;else if (!ignoreNextChar && charAtI == ")" && !openedApos) parenthesis.open--;else ignoreNextChar = false;
        }
          ...
          default:
            {
              ...
              if (arrIn[0].charAt(0) === "~" && typeof (_sliceInstanceProperty(_context = arrIn[0]).call(_context, 1) * 1) === "number") {
                typeIn = "Integer"; // change all array values

                _forEachInstanceProperty(arrIn).call(arrIn, function (e, i) {
                  arrIn[i] = _sliceInstanceProperty(e).call(e, 1);
                });
              }
            }
        }
      ...
      case " ":
        ...
        if (queryString.substring(i, i + 5).toUpperCase() == " AND ") {
          ...
          closeOperator = "Or";
          i += 3;
        } else if (_sliceInstanceProperty(queryString).call(queryString, i, i + 6).toUpperCase() == " LIKE ") {
          i += 5;
          factory.push("<Contains>");
          closeTag = "</Contains>";
        } else if (_sliceInstanceProperty(queryString).call(queryString, i, i + 4).toUpperCase() == " IN ") {
          i += 3;
          factory.push("<In>");
          closeTag = "</In>";
      ...
      case "'":
        ...
        if (escapeChar) word = _cleanString(word); // special words ([Today] and [Me])

        if (word === "[Me]") {
          word = '<UserID Type="Integer" />';
          type = "Integer";
        } else if (_sliceInstanceProperty(word).call(word, 0, 6) == "[Today") {
          type = "DateTime"; // find the offset if defined

          word = '<Today OffsetDays="' + 1 * _sliceInstanceProperty(word).call(word, 6, -1) + '" />';
        }
