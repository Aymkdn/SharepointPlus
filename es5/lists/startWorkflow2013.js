...
export default function startWorkflow2013(setup) {
  ...
  return new _Promise(function (prom_resolve, prom_reject) {
    ...
    SP.SOD.executeFunc("sp.js", "SP.ClientContext", function () {
      ...
      SP.SOD.executeFunc('sp.workflowservices.js', "SP.WorkflowServices.WorkflowServicesManager", function () {
        ...
        context.load(servicesManager); // list the existing workflows
        ...
        context.executeQueryAsync(function () {
          ...
          var initiationParams = {},
              i,
              passed = false;
          var workflowName = setup.workflowName.toLowerCase(); // set the parameters
          ...
          if (setup.platformType == 2010) {
            ...
            while (subsEnum.moveNext()) {
              ...
              if (sub.get_name().toLowerCase() === workflowName) {
                if (setup.ID) servicesManager.getWorkflowInstanceService().startWorkflowOnListItem(sub, setup.ID, initiationParams);else servicesManager.getWorkflowInstanceService().startWorkflow(sub, initiationParams);
                context.executeQueryAsync(function () {
                  prom_resolve();
                }, function (sender, args) {
