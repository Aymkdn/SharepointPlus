 ...
 */
function closeModalDialog(dialogResult, returnValue) {
  var fct = function fct() {
    ...
    if ((0, _typeof2.default)(dialogResult) === "object" && typeof dialogResult.type !== "undefined" && dialogResult.type === "modalDialog") {
      ...
      dialogResult.modal.close(md); // if it's a wait screen, then we need to remove the <style> using options.dialogReturnValueCallBack
      ...
      if (typeof window.top._SP_MODALDIALOG !== "undefined") {
        ...
        if (md.length > 0) {
          md = md[md.length - 1]; // close has only one parameter
