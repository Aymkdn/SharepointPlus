...
*/
export default async function add(items, options) {
  try {
    if (!this.listID) throw "[SharepointPlus 'add'] the list ID/Name is required.";
    if (!this.url) throw "[SharepointPlus 'add'] not able to find the URL!"; // we cannot determine the url
    ...
    let itemsLength=items.length, nextPacket, cutted, itemKey, itemValue, it, i;
    ...
    if (itemsLength > setup.packetsize) {
      ...
      global._SP_ADD_PROGRESSVAR[setup.progressVar.eventID] = setup => add.call(this,nextPacket,setup);
      items = cutted;
    ...
    let updates = '<Batch OnError="Continue" ListVersion="1"  ViewName=""'+(setup.rootFolder?' RootFolder="'+setup.rootFolder+'"':'')+'>';
          ...
          switch (itemKey) {
            case "RecurrenceData": {
              ...
              if (typeof items[i]['TimeZone'] === 'undefined') {
                ...
                if (global._SP_CACHE_TIMEZONEINFO[this.url]) updates += "<Field Name='TimeZone'>"+global._SP_CACHE_TIMEZONEINFO[this.url].ID+"</Field>";
                else {
                  let info = await getTimeZoneInfo.call(this, {url:this.url});
    ...
    if (setup.progressVar.current < setup.progressVar.max) {
      if (global._SP_ADD_PROGRESSVAR[setup.progressVar.eventID]) {
        // if we have "breakOnFailure", then we check the added items
        if (setup.breakOnFailure && failed.length>0) {
          // clean memory
          global._SP_ADD_PROGRESSVAR[setup.progressVar.eventID]=undefined;
          // and resolve by passing the items resolution
          return Promise.resolve({passed:passed, failed:failed});
        }
        return global._SP_ADD_PROGRESSVAR[setup.progressVar.eventID](setup);
      }
    } else {
