  ...
  var SharepointPlus = {
    ...
    */
    ...
    /**
      @name $SP.getURL
      @function
      @description Return the URL base website
      @example
      $SP.getURL(); // -> http://my.sharepoi.nt/my/dir/
     */
    getURL:function() {
      if (this.url==undefined && typeof L_Menu_BaseUrl=="undefined") return undefined;
      return this.url || (window.location.protocol +"//"+ window.location.host + L_Menu_BaseUrl);
    },
    /**
      @name $SP.parse
      @function
    ...
    */
    ...
    /**
      ...
      @param {Object} [setup] Options (see below)
        ...
        @param {Boolean} [setup.forceOWS=false] If you want to force the script to use Lists.asmx instead of dspsts.asmx (shorter data -- it will swtich to TRUE automatically for some cases)
        @param {Boolean} [setup.whereCAML=false] If you want to pass a WHERE clause with CAML Syntax only
      ...
      $SP.list("My List").get({fields:"Title", where:"Fiscal_x0020_Week > 30 AND Fiscal_x0020_Week &lt; 50 AND Name = 'O\\'Sullivan, James'"}),function(row) {
      ...
      $SP.list("{ABCD-1234-XXXX-0000}").get({fields:"Title", where:"Author = '[Me]'"},function(row) {
      ...
      // so the function will grab the view information and will get the data from the list with "Author = '[Me]'" and adding the view's where clause
      $SP.list("My List","http://my.sharepoint.com/my/site/").get({view:"Marketing View", where:"Author = '[Me]'"}, function(data) {
    ...
    */
    get:function(list, setup, fct) {
      ...
      // default values
      setup           = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'get': not able to find the URL!"; // we cannot determine the url
      ...
      // if view is defined, then we need to find the view details first
      if (setup.view != "") {
        var _this=this;
        this.view(setup.view,{url:setup.url},function(data) {
          var where = _this.parse(setup.where) + data.whereCAML;
      ...
      if (useOWS) {
        ...
        jQuery.ajax({type: "POST",
                     ...
                     dataType: "xml",
                     success:function(data) { if (typeof fct == "function") fct.call(_this,data.getElementsByTagName('Row')); else aReturn = data.getElementsByTagName('Row'); }
                   });
      }
    ...
    */
    getAttachment:function(list, itemID, setup, fct) {
      ...
      setup           = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'getAttachment': not able to find the URL!"; // we cannot determine the url
    ...
    */
    info:function(list, setup, fct) {
      ...
      setup           = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'info': not able to find the URL!"; // we cannot determine the url
    ...
    /**
      ...
      @param {String} [listID] The list ID (depreciate -- please use the syntax $SP.list(listID).view())
      ...
      @example
      $SP.list("{ABCD-1234-XXXX-0000}").view("{1487-ACDD-XXXX-0000}",function(data) {
        for (var i=0; i&lt;data.fields.length; i++) console.log("Column "+i+": "+data.fields[i]);
      });
      
      $SP.list("My List","http://my.sharepoint.com/my/site").view("My Beautiful View",function(data) {
        for (var i=0; i&lt;data.fields.length; i++) console.log("Column "+i+": "+data.fields[i]);
      });
    */
    view:function(list, viewID, setup, fct) {
      switch (arguments.length) {
          ...
          case 3: if (typeof viewID=="string" && typeof setup == "function") return this.view(this.listID, list, {}, setup);
                  else if (typeof list=="string" && typeof viewID == "object") return this.view(this.listID, list, viewID, setup);
          case 4: this.listID=list;
      }
      ...
      setup           = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'view': not able to find the URL!"; // we cannot determine the url
      viewID            = viewID || "";
      
      // if viewID is not an ID but a name
      if (viewID.charAt(0) != '{') {
        this.views({url:setup.url},function(views) {
          var found=false;
          for (var i=views.length; i--;) {
            if (views[i]["Name"]==viewID) {
              $SP.list(list).view(views[i]["ID"],setup,fct);
              found=true;
              break;
            }
          }
          if (!found) throw "Error 'view': not able to find the view called '"+viewID+"' at "+setup.url;
        });
        return this;
      }
    ...
    */
    views:function(list, setup, fct) {
      ...
      setup           = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'views': not able to find the URL!"; // we cannot determine the url
      fct = fct || function(){};
            
      // check if we didn't save this information before
      var savedViews = $('body').data('sp-views');
      if (savedViews!=undefined) {
        for (var i=savedViews.length; i--;) {
          if (savedViews[i].url==setup.url) { fct(savedViews[i].data); return this }
        }
      } else savedViews=[];
      ...
      jQuery.ajax({type: "POST",
                   ...
                   success:function(data) {
                    ...
                    savedViews.push({url:setup.url,data:aReturn});
                    $('body').data('sp-views',savedViews);
                    fct(aReturn);
                   }
                 });
      return this;
    ...
    */
    lists:function(setup, fct) {
      ...
      setup           = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'lists': not able to find the URL!"; // we cannot determine the url
      ...
      // check if we didn't save this information before
      var savedLists = $('body').data('sp-lists');
      if (savedLists!=undefined) {
        for (var i=savedLists.length; i--;) {
          if (savedLists[i].url==setup.url) {
      ...
      jQuery.ajax({type: "POST",
                   ...
                   success:function(data) {
                    ...
                    // save the data into the DOM for later usage
                    savedLists.push({url:setup.url,data:aReturn});
                    $('body').data('sp-lists',savedLists);
                    if (typeof fct == "function") fct(aReturn);
                   }
                 });
      if (typeof fct == "undefined") {
    ...
    */
    add:function(list, items, setup) {
      ...
      setup         = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'add': not able to find the URL!"; // we cannot determine the url
    ...
    */
    update:function(list, items, setup) {
      ...
      setup         = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'update': not able to find the URL!"; // we cannot determine the url
      setup.async   = (setup.async == undefined) ? true : setup.async;
      setup.where   = setup.where || "";
      setup.success = setup.success || (function() {});
      setup.error   = setup.error || (function() {});
    ...
    */
    moderate:function(list, items, setup) {
      ...
      setup         = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'moderate': not able to find the URL!"; // we cannot determine the url
      setup.async   = (setup.async == undefined) ? true : setup.async;
      setup.success = setup.success || (function() {});
      setup.error   = setup.error || (function() {});
      setup.after   = setup.after || (function() {});
    ...
    */
    remove:function(list, items, setup) {
      ...
      setup         = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'remove': not able to find the URL!"; // we cannot determine the url
      setup.async   = (setup.async == undefined) ? true : setup.async;
      setup.success = setup.success || (function() {});
      setup.error   = setup.error || (function() {});
      setup.after   = setup.after || (function() {});
    ...
    */
    people:function(username, setup, fct) {
      ...
      setup         = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'people': not able to find the URL!"; // we cannot determine the url
      fct           = fct || (function() {});
      username      = username || "";
    ...
    */
    addressbook:function(username, setup, fct) {
      ...
      setup         = setup || {};
      setup.url       = setup.url || this.getURL();
      if (setup.url == undefined) throw "Error 'addressbook': not able to find the URL!"; // we cannot determine the url
