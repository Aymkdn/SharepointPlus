  ...
  SharepointPlus.prototype = {
    ...
    */
    ...
    /**
      ...
      @example
      $SP()._getURL(); // -> http://my.sharepoi.nt/my/dir/
     */
    _getURL:function() {
      if (this.url===undefined && typeof L_Menu_BaseUrl==="undefined" && $('body').data('sp-currenturl')===undefined) {
        ...
        jQuery.ajax({
          ...
          success:function(data) {
            ...
            if (result.length) {
              _this.url = result[0].firstChild.nodeValue;
              // save the result for future use in the page
              $('body').data('sp-currenturl',_this.url);
            }
            _this.needQueue=false;
          }
        });
      } else this.url = this.url || $('body').data('sp-currenturl') || (window.location.protocol +"//"+ window.location.host + L_Menu_BaseUrl);
    },
    /**
      @ignore
      @name $SP()._addInQueue
    ...
    */
    ...
    /**
      ...
      @param {String} [listID] (depreciated since v3.0 -- use list() instead) The list Name or ID
      @param {Object} [setup] Options (see below)
        ...
        @param {Boolean} [setup.forceOWS=true] (depreciated since v3.0 -- it will be always TRUE) If you want to force the script to use Lists.asmx instead of dspsts.asmx (works only for Sharepoint 2007)
...
//      if (setup.whereCAML!==true) setup.whereCAML = (setup.view!="");
      ...
      if (useOWS) {
        ...
        jQuery.ajax({type: "POST",
                     ...
                     success:function(data) {
                            ...
                            } else if (typeof fct == "function") fct.call(_this,aReturn);
                      }
                   });
      } /*else {
        body = "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
    ...
    /**
      ...
      @param {Object} [setup] Options (see below)
        ...
        @param {String} [setup.after=function(){}] A callback function that will be triggered after the task
    ...
    */
    ...
    /**
      ...
      @param {String} [listID] (depreciated since v3.0 -- use list() instead) The list ID/Name
      @param {String} itemID The item IDs separated by a coma
      @param {Object} [setup] Options (see below)
        @param {String} [setup.url='current website'] (depreciated since v3.0 -- use list() instead) The website url
      @param {Function} [result] A function with the data from the request as first argument
    ...
    */
    view:function(viewID, fct) {
      ...
      var savedView = $('body').data('sp-view');
    ...
    /**
      ...
      @param {String} [listID] (depreciated since v3.0 -- use list() instead) The list ID
      @param {Object} [setup] Options (see below)
        @param {String} [setup.url='current website'] (depreciated since v3.0 -- use list() instead) The website url
      @param {Function} [function()] A function with the data from the request as first argument
    ...
    */
    ...
    /**
      ...
      @param {Object} [setup] Options (see below)
        @param {String} [setup.url='current website'] (depreciated since v3.0 -- use list() instead) The website url
        @param {Boolean} [setup.async=true] (depreciated since v3.0) Determines if we want asynchrous request
    ...
    */
    ...
    /**
      ...
      $SP().people("john_doe",{url:"http://my.si.te/subdir/"}, function(people) {
        for (var i=0; i &lt; people.length; i++) console.log(people[i]+" = "+people[people[i]]); // -> "Roadmap Admin", "Global Viewers", ...
      });
    */
      ...
      jQuery.ajax({type:"POST",
                   ...
                   success:function(data) {
                     ...
                     500: function(req) {
                       ...
                       if (error.length===1) throw "Error 'usergroups': "+error[0].firstChild.nodeValue;
                     }
                   }
                 });
      return this;
